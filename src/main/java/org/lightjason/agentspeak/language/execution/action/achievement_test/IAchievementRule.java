...
abstract class IAchievementRule<T extends ITerm> extends IBaseExecution<T>
{
    ...
    protected static IFuzzyValue<Boolean> execute( final IContext p_context, final ILiteral p_value, final boolean p_parallel )
    {
        ...
        return (
            ...
            final Set<IVariable<?>> l_variables = p_context.getAgent().unifier().literal( i.getIdentifier(), l_unified );
            ...
            return new ImmutableTriple<>( p_context.getAgent().fuzzy().getDefuzzyfication().defuzzify( l_return ), l_return, l_variables );
