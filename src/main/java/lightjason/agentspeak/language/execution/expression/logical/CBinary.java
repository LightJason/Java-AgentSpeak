...
public final class CBinary extends IBaseBinary
{
    ...
    )
    {
        ...
        switch ( m_operator )
        {
            ...
            case AND:
                p_return.add( CRawTerm.from( lightjason.agentspeak.language.CCommon.<Boolean, ITerm>getRawValue( l_argument.get( 0 ) )
                                             && lightjason.agentspeak.language.CCommon.<Boolean, ITerm>getRawValue( l_argument.get( 1 ) ) ) );
                return CFuzzyValue.from( true );
    ...
    private boolean fastCheck( final List<ITerm> p_argument, final List<ITerm> p_return )
    {
        ...
        {
            case AND:
                l_result = !lightjason.agentspeak.language.CCommon.<Boolean, ITerm>getRawValue( p_argument.get( 0 ) );
                break;
